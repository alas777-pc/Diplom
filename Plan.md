...
1. Открытие и отображение главной страницы (ID 18).
2. Свернуть/развернуть раздел новостей (ID 19).
3. Переход к странице с новостями (ID 20).
...
1. Создание новости с оставлением поле заголовок пустым (ID 41).
2. Создание новости без выбора категории (ID 42).
3. Создание новости без выбора даты публикации (ID 43).
4. Создание новости с оставлением поле описание пустым (ID 44).
...
1. Свернуть/развернуть отдельную  цитату (ID 56).
   
### Тест-кейсы для проведения функционального тестирования вкладки "О приложении" (About) мобильного приложения "Мобильный хоспис":

1. Переход по ссылке Правила использования (ID 58).
2. Переход по ссылке Политика конфедециальности (ID 59).
